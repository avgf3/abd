#!/usr/bin/env python3
"""
Frame 18 Animation - CRISP Movement (NO BLUR)
- Extract wings properly
- Move with ZERO blur/ghosting
- Sharp, crisp, clean movement
"""
...
def extract_lower_wings(img):
    """
    Extract lower wings completely
    """
...
def move_wings_crisp(wings_img, frame_num, total_frames):
    """
    Move wings with CRISP quality - no blur
    """
    img_array = np.array(wings_img, dtype=np.float64)
    height, width = img_array.shape[:2]
    
    phase = (frame_num / total_frames) * 2 * math.pi
    base_movement = math.sin(phase) * 25  # Same 25 pixels
    ...
    for y in range(lower_start, height):
        vertical_factor = ((y - lower_start) / (height - lower_start)) ** 1.3
    ...
    # CRISP interpolation - order 3 (cubic) is better for sharpness than 5
    ...
    for channel in range(img_array.shape[2]):
        moved_array[:, :, channel] = ndimage.map_coordinates(
            img_array[:, :, channel],
            [new_yy, xx],
            order=3,  # Cubic - sharper than quintic
            mode='constant',
            prefilter=False,  # No prefilter for crisp edges
            cval=0
        )
...
def create_crisp_animation(input_path, output_path, num_frames=70, duration=43):
    """
    Create CRISP animation - no blur
    """
    print("\n" + "✨ " + "="*70)
    print("✨  FRAME 18 - CRISP SHARP (بدون تمويه)")
    print("✨ " + "="*70 + "\n")
    ...
    print("   ✓ Wings extracted (crisp)\n")
    
    frames = []
    
    print(f"🎬 Creating {num_frames} crisp frames...\n")
    ...
    for i in range(num_frames):
        ...
        # Move wings (crisp)
        moved_wings = move_wings_crisp(wings, i, num_frames)
    ...
    print("✨ Improvements:")
    print("   ✓ Cubic interpolation (sharper)")
    print("   ✓ NO prefilter (crisp edges)")
    print("   ✓ NO smoothing blur")
    print("   ✓ Clean extraction")
    print("   ✓ 25 pixels movement")
    print("   ✓ NO ghosting/blur\n")
    
    print("✨ CRISP AND CLEAN!")
    print("="*70 + "\n")
