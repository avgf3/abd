...
export interface IStorage {
  ...
  // Moderation operations - نظام الإشراف المتكامل
  muteUser(userId: number, durationMinutes: number, reason: string, moderatorId: number): Promise<boolean>;
  unmuteUser(userId: number, moderatorId: number): Promise<boolean>;
  kickUser(userId: number, durationMinutes: number, reason: string, moderatorId: number): Promise<boolean>;
  banUser(userId: number, reason: string, moderatorId: number, ipAddress?: string, deviceId?: string): Promise<boolean>;
  blockUser(userId: number, reason: string, moderatorId: number, ipAddress?: string, deviceId?: string): Promise<boolean>;
  promoteUser(userId: number, newRole: string, moderatorId: number): Promise<boolean>;
  demoteUser(userId: number, newRole: string, moderatorId: number): Promise<boolean>;
  getUserModerationStatus(userId: number): Promise<any>;
...
export class MixedStorage implements IStorage {
  ...
  // ========================= نظام الإشراف المتكامل =========================

  async muteUser(userId: number, durationMinutes: number, reason: string, moderatorId: number): Promise<boolean> {
    try {
      const muteExpiry = new Date(Date.now() + durationMinutes * 60000);
      
      // تحديث في الذاكرة للضيوف
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.isMuted = true;
        memUser.muteExpiry = muteExpiry;
        this.users.set(userId, memUser);
      }
      
      // تحديث في قاعدة البيانات للأعضاء
      try {
        ...
        console.log('User not in database, updated in memory only');
      }
      
      console.log(`🔇 تم كتم المستخدم ${userId} لمدة ${durationMinutes} دقيقة`);
      return true;
    } catch (error) {
  ...
  async unmuteUser(userId: number, moderatorId: number): Promise<boolean> {
    try {
      // تحديث في الذاكرة للضيوف
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.isMuted = false;
        memUser.muteExpiry = null;
        this.users.set(userId, memUser);
      }
      ...
      console.log(`✅ تم إلغاء كتم المستخدم ${userId}`);
      return true;
    } catch (error) {
  ...
  async kickUser(userId: number, durationMinutes: number, reason: string, moderatorId: number): Promise<boolean> {
    try {
      const banExpiry = new Date(Date.now() + durationMinutes * 60000);
      
      // تحديث في الذاكرة للضيوف
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.isBanned = true;
        memUser.banExpiry = banExpiry;
      ...
      console.log(`👋 تم طرد المستخدم ${userId} لمدة ${durationMinutes} دقيقة`);
      return true;
    } catch (error) {
  ...
  async banUser(userId: number, reason: string, moderatorId: number, ipAddress?: string, deviceId?: string): Promise<boolean> {
    try {
      // حظر نهائي - بدون تاريخ انتهاء
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.isBanned = true;
        memUser.banExpiry = null; // حظر نهائي
        memUser.isBlocked = true;
        memUser.isOnline = false;
        memUser.ipAddress = ipAddress || null;
        memUser.deviceId = deviceId || null;
        this.users.set(userId, memUser);
      }
      ...
      console.log(`🚫 تم حظر المستخدم ${userId} نهائياً`);
      return true;
    } catch (error) {
  ...
  async blockUser(userId: number, reason: string, moderatorId: number, ipAddress?: string, deviceId?: string): Promise<boolean> {
    try {
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.isBlocked = true;
      ...
      console.log(`🔒 تم حجب المستخدم ${userId}`);
      return true;
    } catch (error) {
  ...
  async promoteUser(userId: number, newRole: string, moderatorId: number): Promise<boolean> {
    try {
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.userType = newRole as any;
        this.users.set(userId, memUser);
      }
      
      // تحديث في قاعدة البيانات للأعضاء فقط
      try {
        await db.update(users)
          .set({ userType: newRole as any })
          .where(eq(users.id, userId));
      } catch (dbError) {
        console.log('User not in database, updated in memory only');
      }
      
      console.log(`⭐ تم ترقية المستخدم ${userId} إلى ${newRole}`);
      return true;
    } catch (error) {
  ...
  async demoteUser(userId: number, newRole: string, moderatorId: number): Promise<boolean> {
    try {
      const memUser = this.users.get(userId);
      if (memUser) {
        memUser.userType = newRole as any;
      ...
      console.log(`📉 تم تخفيض رتبة المستخدم ${userId} إلى ${newRole}`);
      return true;
    } catch (error) {
      console.error('خطأ في تخفيض رتبة المستخدم:', error);
      return false;
    }
  }

  async getUserModerationStatus(userId: number): Promise<any> {
  ...
  async getModerationActions(): Promise<any[]> {
    // إرجاع قائمة الإجراءات المحفوظة
    // يمكن توسيعها لاحقاً لحفظ تاريخ الإجراءات
    return [];
  }
  ...
  async getModerationStats(): Promise<any> {
    ...
    const stats = {
      ...
      mutedUsers: allUsers.filter(u => u.isMuted).length,
      ...
      moderatorUsers: allUsers.filter(u => u.userType === 'moderator').length,
