...
export class DatabaseService {
  ...
  async createRoom(roomData: Partial<Room>): Promise<Room | null> {
    ...
    try {
      ...
      const room = {
        ...roomData,
        id: generatedId ?? roomData.id, // Use generated ID or provided ID
    ...
    try {
      if (this.type === 'postgresql') {
        ...
        await (this.db as any)
          .update(pgSchema.rooms)
          .set(allowed)
          .where(eq(pgSchema.rooms.id, maybeId));
        const rows = await (this.db as any)
          .select()
          .from(pgSchema.rooms)
          .where(eq(pgSchema.rooms.id, maybeId))
  ...
  async canSendInRoom(
    ...
    try {
      ...
      // Fallback: no per-room moderation implemented
      return { allowed: true };
    } catch (error) {
  ...
  async createBlockedDevice(data: Partial<BlockedDevice>): Promise<BlockedDevice | null> {
    ...
    try {
      if (this.type === 'postgresql') {
        ...
        await (this.db as any).insert(pgSchema.blockedDevices).values(payload);
        return { ...payload };
      }
    } catch (error) {
