...
export function useChat() {
  ...
  const connect = useCallback((user: ChatUser) => {
    ...
    socket.current.on('disconnect', () => {
      ...
      setConnectionError('تم قطع الاتصال بالخادم');
    });

    // استقبال الأحداث من الخادم (مثال: onlineUsers, newMessage, ...)
    socket.current.on('onlineUsers', (users: ChatUser[]) => {
      const filteredUsers = users.filter((chatUser: ChatUser) =>
        !ignoredUsers.has(chatUser.id) && !chatUser.isHidden
      );
      setOnlineUsers(filteredUsers);
    });

    socket.current.on('newMessage', (message: ChatMessage) => {
      if (!ignoredUsers.has(message.senderId)) {
    ...
    socket.current.on('userVisibilityChanged', (message: { userId: number; isHidden: boolean }) => {
      setOnlineUsers(prev => {
        if (message.isHidden) {
    ...
    socket.current.on('privateMessage', (message: ChatMessage) => {
        ...
        if (message.senderId !== user.id) {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('رسالة خاصة جديدة 📱', {
              body: `${message.sender?.username}: ${message.content.slice(0, 50)}...`,
              icon: '/favicon.ico'
            });
          }
        }
    ...
    socket.current.on('userLeft', (userId: number) => {
      setOnlineUsers(prev => prev.filter(u => u.id !== userId));
    });

    socket.current.on('usernameColorChanged', (message: { userId: number; color: string }) => {
    ...
    socket.current.on('theme_update', (message: { userId: number; userTheme: string }) => {
    ...
    socket.current.on('moderationAction', (message: { targetUserId: number; action: string; duration?: number; reason?: string }) => {
    ...
    socket.current.on('typing', (message: { username: string; isTyping: boolean }) => {
    ...
    socket.current.on('notification', (message: { targetUserId: number; notificationType: string; moderatorName?: string; message?: string }) => {
    ...
    socket.current.on('systemMessage', (message: { targetUserId: number; action: string; duration?: number; reason?: string; message?: string }) => {
    ...
    socket.current.on('kicked', (message: { targetUserId: number; duration?: number }) => {
      if (message.targetUserId === user.id) {
        setKickNotification({ 
          show: true, 
    ...
    socket.current.on('blocked', (message: { targetUserId: number; reason?: string }) => {
      if (message.targetUserId === user.id) {
    ...
    socket.current.on('friendRequest', (message: { targetUserId: number; senderId: number; senderUsername: string }) => {
    ...
    socket.current.on('friendRequestAccepted', (message: { targetUserId: number; friendId: number; acceptedBy: string }) => {
    ...
    socket.current.on('promotion', (message: { newRole: string; message: string }) => {
    ...
    socket.current.on('userUpdated', (user: ChatUser) => {
      setOnlineUsers(prev => 
  ...
  const sendMessage = useCallback(async (content: string, messageType: string = 'text') => {
    if (!currentUser || !socket.current || socket.current.connected !== true) {
