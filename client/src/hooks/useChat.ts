  ...
  const connect = useCallback(
    (user: ChatUser) => {
      ...
      try {
        ...
        // معالج فشل إعادة الاتصال النهائي (سلوك صامت بدون إشعار واجهة)
        s.on('reconnect_failed', () => {
          console.warn('⚠️ فشل في إعادة الاتصال بعد عدة محاولات');
          // لا نحدّث واجهة المستخدم للحفاظ على الصمت
        });

        // تحديث حالة الاتصال عند الانفصال - سلوك صامت (لا تغييرات واجهة)
        s.on('disconnect', () => {
          // صامت: لا نغير حالة الواجهة حتى تتم إعادة الاتصال
        });

        // معالجة أخطاء الاتصال - سلوك صامت (log فقط)
        s.on('connect_error', (error) => {
          console.error('❌ خطأ في الاتصال:', error);
          // صامت: لا نحدّث حالة الواجهة
        });
      } catch (error) {
        console.error('خطأ في الاتصال:', error);
