      ...
      <div className="relative w-[90vw] max-w-[20rem] sm:max-w-[22rem] bg-card rounded-xl overflow-hidden shadow-2xl animate-fade-in">
        ...
        <div className="max-h-[70vh] overflow-y-auto bg-background">
          ...
          <ul className="space-y-1">
            {topTen.map((u, idx) => (
              <li key={u.id} className="relative -mx-4 list-none border-b border-border">
                <UnifiedUserCard
                  user={u}
                  variant="normal"
                  ...
                  showRankNumber={idx + 1}
                  showMedal={idx < 3}
                  showDeleteButton={canManage}
                  onUserClick={onUserClick}
                  onDelete={() => handleRemoveVip(u.id)}
                  currentUser={currentUser}
                  enableMenu={true}
                  enableEffects={true}
                  imageSize="small"
                  className="px-4 hover:bg-accent/10 cursor-pointer"
                />
              </li>
            ))}
          </ul>
