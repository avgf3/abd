...
export default function ProfileModal({
  ...
  // جلب بيانات المستخدم كاملة عند فتح المودال أو تغيّر المعرّف لتوحيد العرض بين جميع التبويبات
  useEffect(() => {
    if (!user?.id) return;
    // جلب نسخة كاملة من بيانات المستخدم من الخادم لضمان اكتمال الحقول
    fetchAndUpdateUser(user.id).catch(() => {});
    // ملاحظة: نتجنب إظهار مؤشّر التحميل العام حتى لا نربك المستخدم أثناء القراءة
    // الواجهة تعتمد على localUser فوراً وتتحسّن عند وصول البيانات
  }, [user?.id]);
