    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            result.rows.forEach((user, index) => {
                ...
                console.log(`   📅 تاريخ الإنشاء: ${user.created_at ? new Date(user.created_at).toLocaleString('ar-SA') : 'غير محدد'}`);
                console.log(`   📅 تاريخ الانضمام: ${user.join_date ? new Date(user.join_date).toLocaleString('ar-SA') : 'غير محدد'}`);
            ...
            recent.forEach((user, index) => {
                console.log(`   ${index + 1}. ${user.username} (${user.user_type}) - ${user.created_at ? new Date(user.created_at).toLocaleDateString('ar-SA') : 'تاريخ غير محدد'}`);
            });
        ...
        if (loginableUsers.length > 0) {
            loginableUsers.forEach((user, index) => {
                console.log(`   ${index + 1}. اسم المستخدم: "${user.username}" | كلمة المرور: "${user.password}" | النوع: ${user.user_type}`);
            });
        } else {
