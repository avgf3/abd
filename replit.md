...
### Complete Personal Ignore System (January 11, 2025)
- **Personal Ignore Feature**: Full ignore system allowing users to hide messages from specific users
- **Database Integration**: Created ignored_users table with PostgreSQL for persistent storage
- **API Endpoints**: Complete REST API for ignore operations (add, remove, list, check)
- **UI Components**: Dedicated IgnorePanel for managing ignored users with search and removal
- **Real-time Filtering**: Automatic message filtering for ignored users in public chat
- **Settings Integration**: Added ignore list access through settings menu
- **Persistent Storage**: Ignore preferences saved in database and loaded on login
- **UserPopup Integration**: Direct ignore functionality from user context menu
- **Distinction from Admin Block**: Personal ignore (user choice) separate from admin moderation blocks
